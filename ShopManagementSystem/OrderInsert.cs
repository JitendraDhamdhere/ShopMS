...
namespace ShopManagementSystem
{
    public partial class OrderInsert : Form
    {
        ...
        private void AutoIdGeneration()
        {
            ...
            try
            {
                string useDbSql = "SELECT ISNULL(MAX(ORD_ID), 0) + 1 FROM MYORDER;";
                SqlCommand cmd = new SqlCommand(useDbSql, con);

                object result = cmd.ExecuteScalar();
                if (result != null && int.TryParse(result.ToString(), out Num))
                {
                    OrderID.Text = Num.ToString(); ; // Set the new customer ID in the form
                }
                else
                {
                    MessageBox.Show("Failed to generate a new Order ID.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private void Save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DateTime orderDate;
                if (DateTime.TryParse(date.Text, out orderDate))
                {
